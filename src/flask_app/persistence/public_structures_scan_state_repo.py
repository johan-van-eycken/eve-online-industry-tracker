...
from classes.database_models import PublicStructuresScanStateModel
...
def get_state(session) -> PublicStructuresScanStateModel:
    row = session.execute(
        select(PublicStructuresScanStateModel).where(PublicStructuresScanStateModel.id == _STATE_ID)
    ).scalar_one_or_none()
    if row is None:
...
def get_last_completed_at(session) -> Optional[datetime]:
