...
from classes.asset_provenance import FifoLot, fifo_allocate_cost, fifo_allocate_cost_breakdown
    ...
    *,
    ...
    def _buy_cost_for_type(
        ...
        *,
        ...
        # If we need market pricing for unknown inventory or shortfall, but market price is missing,
        # then the buy cost is unknown unless FIFO-priced inventory fully covers the requirement.
    ...
    def _blueprint_bpo_buy_cost(bp_type_id: int) -> float | None:
        # Best-effort: treat blueprint type_id itself as market-traded BPO.
    ...
    def _is_reaction_only(product_type_id: int) -> bool:
        if int(product_type_id) in mfg_product_to_bps:
            return False
        # If a reaction blueprint exists producing it, consider it reaction-only.
        return int(product_type_id) in reaction_product_type_ids

    def _compute_eiv_total_for_inputs(inputs_me0: list[dict]) -> float | None:
        """Compute EIV total (job-fee basis) for a list of inputs.

        IMPORTANT: This uses ME0 quantities with adjusted prices (client behavior).
        """
    ...
    def _plan_one(type_id: int, qty: int, *, depth: int, path: set[int]) -> dict:
        ...
        # "take" is a buy-variant meaning: we already have the stock and it's cheaper than buying now.
        # Only use it when inventory fully covers the requirement and FIFO has a priced basis.
        take_possible = False
        try:
        ...
        # Submanufacturing efficiency rules:
        # - If an owned blueprint exists (BPC preferred over BPO), use its ME/TE.
        # - Otherwise (unowned), estimate as if run on a max-researched BPC (ME 10% / TE 20%).
        # NOTE: Job-fee EIV remains ME0-based (client behavior).
        ...
        # Plan children (inputs) recursively.
        children: list[dict] = []
        ...
        for inp in mfg_materials:
            ...
            # Apply ME reduction to the required quantity (rounding up like the client).
        ...
        # Build cost = sum(child effective costs) + manufacturing job fee.
        ...
        # Build time (simple, conservative): sum of children times + this node manufacturing time (+ optional copy time).
        # This assumes jobs are done sequentially; we can refine later if you want parallel scheduling.
        ...
        manufacturing_time_seconds = max(
            0.0,
            float(mfg_time_per_run) * float(runs_needed) * float(te_multiplier_used),
        )
        ...
        # Blueprint acquisition info (if not owned).
        owns_bp = blueprint_type_id in owned_bps
        bpo_buy_cost = _blueprint_bpo_buy_cost(blueprint_type_id) if not owns_bp else None

        # Optional: estimate copy/research overhead for creating a BPC.
        # Requirements:
        # - Blueprint (BPO) buy cost: informational only
        # - ME/TE research: informational only
        # - Copy job cost + duration: INCLUDED in build totals when blueprint is not owned
        ...
        if copying_time_s > 0 and runs_needed > 0:
            ...
            # Include copy overhead in totals only when the blueprint isn't owned.
            if not owns_bp:
        ...
        # Decision at this node: compare buy cost vs build cost.
        if buy_cost is not None and total_build_cost is not None:
            ...
            # Time: if we choose buy, treat it as 0 time in our planner.
            node["effective_time_seconds"] = float(total_build_time_seconds) if (rec == "build" and total_build_time_seconds is not None) else 0.0
        else:
            # If we can't compare, prefer build only if we can compute build cost and buy is unknown.
            if total_build_cost is not None and buy_cost is None:
                ...
                node["effective_time_seconds"] = float(total_build_time_seconds) if total_build_time_seconds is not None else None
