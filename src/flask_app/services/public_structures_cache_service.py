...
from flask_app.persistence import public_structures_repo
from flask_app.persistence import public_structures_scan_state_repo
...
def _ensure_table_exists(session) -> None:
    try:
        ...
        # Best-effort; if it fails, normal inserts will surface the error.
        pass
...
def _ensure_scan_state_table_exists(session) -> None:
    try:
        ...
        from classes.database_models import PublicStructuresScanStateModel  # noqa: WPS433
...
def get_cached_public_structures(system_id: int, *, ttl_seconds: int) -> tuple[list[dict], bool]:
...
def refresh_public_structures_for_system(
    ...
    *,
    ...
    # Important: don't hold a db_app connection while doing the (concurrent) ESI scan.
...
def trigger_refresh_public_structures_for_system(
    ...
    *,
    ...
    threading.Thread(
        target=_run,
        daemon=True,
        name=f"public-structures-refresh-{system_id}",
    ).start()
    return True
...
def trigger_global_public_structures_scan(
    *,
    ...
    This is best-effort and can take a long time. It runs in slices and resumes using a persisted cursor.
    """
    if state.db_app is None or state.esi_service is None:
        return False
    ...
    threading.Thread(target=_run, daemon=True, name="public-structures-global-scan").start()
    return True
...
def _global_scan_loop(
    *,
    ...
    # 1) Fetch list of accessible structure IDs.
    ...
    # 3) Run slices until we complete a full pass.
    while True:
        ...
        def fetch_one(structure_id: int):
            try:
                data = state.esi_service._esi_client.esi_get(
        ...
        # Progressive submission within time budget.
        from concurrent.futures import FIRST_COMPLETED, ThreadPoolExecutor, wait  # noqa: WPS433
            ...
            while in_flight:
                ...
                for fut in done:
                    ...
                    if len(rows_batch) >= batch_size:
                        _commit_global_batch(rows_batch, cursor + attempted_in_slice)
        ...
        # Advance cursor by attempted.
        cursor = min(total, cursor + attempted_in_slice)

        # Persist cursor even if we didn't write any rows this slice.
        _persist_global_cursor(cursor)

        # Pause between slices.
        if pause_seconds > 0:
            time.sleep(pause_seconds)

        # Safety: if we made no progress in a slice, avoid a tight loop.
        if attempted_in_slice == 0:
            time.sleep(max(1.0, pause_seconds))


def _persist_global_cursor(cursor: int) -> None:
...
def _commit_global_batch(rows: list[dict], new_cursor: int) -> None:
    if state.db_app is None:
        return
    session = state.db_app.Session()
    try:
