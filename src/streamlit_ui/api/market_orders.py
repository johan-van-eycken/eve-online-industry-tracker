...
@st.cache_data(ttl=3600)
def fetch_corp_market_orders() -> dict[str, Any]:
    response = api_get("/corporations/market_orders?refresh=0", timeout_seconds=60) or {}
    return response if isinstance(response, dict) else {}


def refresh_corp_market_orders() -> None:
    response = api_get("/corporations/market_orders?refresh=1", timeout_seconds=120) or {}
    if isinstance(response, dict) and response.get("status") not in {None, "success"}:
