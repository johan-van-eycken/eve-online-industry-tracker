...
from streamlit_ui.api.characters import build_character_options, fetch_characters
...
from streamlit_ui.components.formatters import format_isk, format_date, format_date_into_age, type_icon_url
...
def _render_asset_dataframe(df: pd.DataFrame, *, height: int, prefer_container_names: bool = False) -> None:
    """Render assets via st.dataframe — works reliably inside st.expander."""
    df_display = build_asset_display_frame(df, prefer_container_names=prefer_container_names)
    if df_display.empty:
        st.info("No items.")
        return
