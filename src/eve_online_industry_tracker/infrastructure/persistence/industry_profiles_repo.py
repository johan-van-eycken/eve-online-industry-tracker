...
from eve_online_industry_tracker.db_models import IndustryProfilesModel

from eve_online_industry_tracker.domain.industry_profile import IndustryProfile


def list_by_character_id(session, character_id: int) -> list[IndustryProfile]:
    rows = session.query(IndustryProfilesModel).filter(IndustryProfilesModel.character_id == character_id).all()
    return [IndustryProfile.from_model(r) for r in rows]


def get_by_id(session, profile_id: int) -> IndustryProfile | None:
    row = session.query(IndustryProfilesModel).filter(IndustryProfilesModel.id == int(profile_id)).first()
...
def create(session, data: dict[str, Any]) -> int:
    profile = IndustryProfilesModel(
        ...
        scc_surcharge=data.get("scc_surcharge"),
        facility_cost_bonus=data.get("facility_cost_bonus"),
...
def update(session, profile_id: int, data: dict[str, Any]) -> None:
    profile = session.query(IndustryProfilesModel).filter(IndustryProfilesModel.id == profile_id).first()
    if not profile:
