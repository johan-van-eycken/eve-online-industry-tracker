...
def render():
        ...
        for profile in profiles:
            with st.expander(
                ...
                with col1:
                    ...
                    st.write(
                        f"**ME Bonus:** {profile['material_efficiency_bonus']}%"
                    )
                    st.write(f"**TE Bonus:** {profile['time_efficiency_bonus']}%")

                with col2:
                    st.write(
                        f"**Cost Index:** {profile['manufacturing_cost_index']:.4f}"
                    )
                    st.write(
                        f"**Installation Tax:** {profile['installation_cost_modifier']:.2%}"
                    )
                    st.write(
                        f"**Rig Material Bonus:** {profile['structure_rig_material_bonus']:.2%}"
                    )
                    st.write(
                        f"**Rig Time Bonus:** {profile['structure_rig_time_bonus']:.2%}"
                    )

                with col3:
                    if st.button("Delete", key=f"delete_{profile['id']}"):
                        delete_response = api_delete(
                            f"/industry_profiles/{profile['id']}"
                        )
    ...
    # Create new profile - Multi-step form
    st.markdown("### Create New Profile")

    # Step 1: Profile Name and System Selection
    if st.session_state.profile_step == 1:
        ...
        profile_name = st.text_input(
            ...
            value=st.session_state.profile_data.get("profile_name", ""),
        )
        is_default = st.checkbox(
            "Set as default",
            value=st.session_state.profile_data.get("is_default", False),
        )

        st.markdown("**Select Solar System**")

        # Fetch solar systems
        solar_systems_response = api_get("/solar_systems")
        ...
        selected_system_id = st.selectbox(
            ...
            key="system_select",
            index=(
                list(system_options.keys()).index(
                    st.session_state.profile_data.get("system_id")
                )
                if st.session_state.profile_data.get("system_id")
                in system_options.keys()
                else 0
            ),
        )

        selected_system = next(
            (s for s in solar_systems if s["id"] == selected_system_id), None
        )
        ...
        if selected_system:
            st.caption(
                ...
                f"**Security:** {selected_system.get('security_status', 0.0):.1f}"
            )
        ...
        with col1:
            if st.button("Next →", type="primary", use_container_width=True):
                ...
                elif not selected_system:
                    st.error("Please select a solar system")
                else:
                    st.session_state.profile_data.update(
                        {
                            ...
                            "region_id": selected_system["region_id"],
                        }
                    )
    ...
    # Step 2: Station Selection
        ...
        st.markdown("**Step 2: Station Selection**")

        st.info(
            f"📍 **System:** {st.session_state.profile_data.get('system_name')} | "
            f"**Profile:** {st.session_state.profile_data.get('profile_name')}"
        )

        system_id = st.session_state.profile_data.get("system_id")

        # Fetch NPC stations
        npc_stations_response = api_get(f"/npc_stations/{system_id}")
        if npc_stations_response is None:
            st.error("Failed to fetch stations.")
            return
        elif npc_stations_response.get("status") != "success":
            st.warning("No NPC stations found in this system.")
            stations = []
        else:
        ...
        selected_station_key = st.selectbox(
            ...
            key="station_select",
            index=(
                list(station_options.keys()).index(
                    str(st.session_state.profile_data.get("station_id", "none"))
                )
                if str(st.session_state.profile_data.get("station_id", "none"))
                in station_options.keys()
                else 0
            ),
        )
        ...
        if selected_station_key != "none":
            ...
            if selected_station:
                ...
                with col1:
                    ...
                    st.write(
                        f"**Reprocessing Efficiency:** {selected_station.get('reprocessing_efficiency', 0):.1%}"
                    )
                with col2:
                    st.write(
                        f"**Manufacturing Factor:** {selected_station.get('manufacturing_factor', 1.0):.2f}"
                    )
                    st.write(
                        f"**Research Factor:** {selected_station.get('research_factor', 1.0):.2f}"
                    )
                ...
                if selected_station_key == "none":
                    ...
                    selected_station = next(
                        (
                            s
                            for s in stations
                            if str(s["station_id"]) == selected_station_key
                        ),
                        None,
                    )
                    st.session_state.profile_data.update(
                        {
                            "station_id": int(selected_station_key),
    ...
    # Step 3: Bonuses and Final Configuration
    elif st.session_state.profile_step == 3:
        st.markdown("**Step 3: Industry Bonuses**")

        st.info(
            f"📍 **System:** {st.session_state.profile_data.get('system_name')} | "
            f"**Location:** {st.session_state.profile_data.get('station_name') or 'Structure'} | "
            f"**Profile:** {st.session_state.profile_data.get('profile_name')}"
        )
        ...
        with col1:
            st.markdown("**Material & Time Efficiency**")
            ...
            time_efficiency_bonus = st.number_input(
                ...
                min_value=0.0,
                max_value=25.0,
                value=st.session_state.profile_data.get("time_efficiency_bonus", 0.0),
                step=0.1,
            )

        with col2:
            st.markdown("**Cost Modifiers**")
        ...
        with col4:
            structure_rig_time_bonus = st.number_input(
                ...
                min_value=0.0,
                max_value=50.0,
                value=st.session_state.profile_data.get("structure_rig_time_bonus", 0.0),
                step=0.1,
            )
        with col5:
            structure_rig_cost_bonus = st.number_input(
                ...
                min_value=0.0,
                max_value=50.0,
                value=st.session_state.profile_data.get("structure_rig_cost_bonus", 0.0),
                step=0.1,
            )
            ...
            if st.button("Create Profile", type="primary", use_container_width=True):
                create_response = api_post(
                    "/industry_profiles",
                    {
                        ...
                        "profile_name": st.session_state.profile_data.get("profile_name"),
                        "is_default": st.session_state.profile_data.get("is_default", False),
                        "location_id": st.session_state.profile_data.get("station_id"),
                        "location_name": st.session_state.profile_data.get("station_name"),
                        "location_type": st.session_state.profile_data.get("location_type"),
