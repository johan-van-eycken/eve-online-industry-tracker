...
def render():
    ...
    if buy_orders:
        ...
        df = df.reset_index(drop=True).sort_values(by=['Type', 'Price'], ascending=[True, False])
        ...
        st.write(
            "Active Orders: <strong>{}</strong>&nbsp;&nbsp;Total ISK: <strong>{}</strong>&nbsp;&nbsp;Total Escrow: <strong>{}</strong>".format(
                len(df), format_isk_short(df['Total Price'].sum()), format_isk_short(df['Escrow Remaining'].sum())
            ),
            unsafe_allow_html=True
        )
